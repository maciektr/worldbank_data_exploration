...
def load_dataset(indicators=None, years=slice(2000, 2019), nans_threshold=2):
    if indicators is None:
        indicators = ['SP.POP.GROW', 'FP.CPI.TOTL.ZG', 'SP.DYN.LE00.IN', 'NE.EXP.GNFS.ZS', 'NY.GDP.MKTP.KD.ZG',
                      'SL.UEM.TOTL.ZS', 'NV.AGR.TOTL.ZS', 'EG.ELC.ACCS.ZS', 'AG.LND.FRST.ZS', 'SH.DYN.MORT',
                      'NY.GDP.TOTL.RT.ZS', 'SP.DYN.TFRT.IN', 'EN.URB.LCTY.UR.ZS', 'TG.VAL.TOTL.GD.ZS',
                      'MS.MIL.XPND.GD.ZS']

    df = get_indicators(indicators)
    df = df.pivot_table(values='Value', index='Year', columns=['Indicator Name', 'Country Name'])
